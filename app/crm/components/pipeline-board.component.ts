...
export class PipelineBoardComponent {
  ...
  dragOverStage = signal<LeadStatus | null>(null);

  leadsFor(stage: LeadStatus): Lead[] {
    return this.leads.filter((lead) => lead.status === stage);
  }

  stageBefore(stage: LeadStatus): LeadStatus | null {
    const index = this.stages.indexOf(stage);
    return index > 0 ? this.stages[index - 1] : null;
  }

  stageAfter(stage: LeadStatus): LeadStatus | null {
    const index = this.stages.indexOf(stage);
    return index >= 0 && index < this.stages.length - 1 ? this.stages[index + 1] : null;
  }

  onDragOver(event: DragEvent, stage: LeadStatus): void {
    event.preventDefault();
    this.dragOverStage.set(stage);
  }

  onDragLeave(stage: LeadStatus): void {
    if (this.dragOverStage() === stage) {
      this.dragOverStage.set(null);
    }
  }

  onDrop(event: DragEvent, stage: LeadStatus): void {
    event.preventDefault();
    this.dragOverStage.set(null);
    const id = event.dataTransfer?.getData('text/plain');
    if (id) {
      this.moveTo.emit({ id, status: stage });
    }
  }
}
